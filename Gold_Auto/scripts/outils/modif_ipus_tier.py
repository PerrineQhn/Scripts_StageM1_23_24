...
def get_total_duration(textgrid_file: str) -> float:
    ...
    """
    textgrid_tier = tgio.openTextgrid(textgrid_file, includeEmptyIntervals=True)
    tier_trans = textgrid_tier.getTier("trans")
    tier_list = tier_trans.entries
    # print(tier_list)
...
def correct_silence_duration(
    ...
    """
    textgrid_ipus = tgio.openTextgrid(ipu_textgrid, includeEmptyIntervals=True)
    new_intervals = []
    ipu_tier = textgrid_ipus.getTier("IPUs")
    ...
    new_intervals = correct_intervals_limite_duration(new_intervals, limite)
    new_intervals = merge_intervals(new_intervals)
    new_intervals = rename_intervals(new_intervals)
    ...
    new_ipus_textgrid.save(output, format="long_textgrid", includeBlankSpaces=True)
