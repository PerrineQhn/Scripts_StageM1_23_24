        ...
        while j < len(sentence):
            ...
            if token in punctuation_list:
                ...
                print(sentence[j], j, sentences_list[idx_sentences_list], idx_sentences_list, i, token_label,
                      ' "\'" in token and "\'" not in token_label LINE 209')
                move_step = 0
            ...
            # Adding hidden characters to new sentence
                ...
                while not (sentences_list[idx_sentences_list].upper() == tier[i][0].upper() and (sentences_list[idx_sentences_list+1].upper() == tier[i+1][0].upper() or tier[i+1][0].upper() == '#')):
                    ...
                    if j < len(sentence) and sentence[j].upper() == "YOU'LL" and tier[i][0].upper() == 'YOU':
                        ...
                        print('\n sentences_list[idx_sentences_list].upper() == "IT\'S" and tier[i+1][0].upper() == "IT":', sentences_list[idx_sentences_list].upper(), sentences_list[idx_sentences_list+1].upper(), sentence[j], tier[i][0].upper(), tier[i+1][0].upper())
                        if sentences_list[idx_sentences_list+1].upper() == "IT'S" and tier[i+1][0].upper() == 'IT':
                            break
    ...
    for gold_file in gold_files:
        ...
        if gold_file.endswith('MG.conllu') or gold_file.endswith('M.conllu'):
            ...
            if non_gold_file in non_gold_files:
                ...
                list_file = [
                    # 'SUD_Naija-NSC-master/WAZL_02_Good-Morning-Nigeria_MG.conllu', #
                    'SUD_Naija-NSC-master/PRT_05_Ghetto-Life_MG.conllu', #
                    # 'SUD_Naija-NSC-master/KAD_12_Mechanic-At-Work_MG.conllu', #
                    ]
