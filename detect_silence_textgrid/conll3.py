...
class Tree(dict):
    ...
    def __repr__(self):
        ...
        for i in self:  # nodes
            representation += (
                str(i) + ": " + self[i].get("t", "_") + "\t" + str(self[i]) + "\n"
            )
        ...
        if self.sentencefeatures:
            for stftkey in sorted(self.sentencefeatures):
                if stftkey == "_comments":
                    ...
                    treestring += (
                        "# " + stftkey + " = " + self.sentencefeatures[stftkey] + "\n"
                    )
                ...
                treestring += (
                    "\t".join(
                        [
                            ...
                            gv,
                            "|".join(
                                [str(g) + ":" + govs.get(g, "_") for g in govk[1:]]
                            )
                            or "_",
                            node.get("misc", "_"),
                        ]
    ...
    for line in conllstring.split("\n"):
        if line.strip():
            if line.strip()[0] == "#":  # comment of conllu
                if "=" in line:
                    tree.sentencefeatures[line.split("=")[0].strip()[1:].strip()] = (
                        "=".join(line.split("=")[1:]).strip()
                    )
                else:
                    tree.sentencefeatures["_comments"] = (
                        tree.sentencefeatures.get("_comments", "")
                        + line.strip()[1:]
                        + "\n"
                    )
                continue
            ...
            if nrCells in [4, 10, 12, 13, 14]:
                ...
                if nrCells == 10:  # standard conll 10 or conllu
                    ...
                    try:
                        ...
                        nr = float(nr)  # handling the 3.1 format for "emtpy nodes"
                    if head.strip() == "_":
                        head = -1
                    else:
                        try:
                    ...
                    if ":" in edeps:  # the enhanced graph is used
                        egov = dict(
                            [
                                (gf.split(":")[0], gf.split(":")[-1])
                                for gf in edeps.split("|")
                            ]
                        )
                    ...
                    if "=" in features:
                        mf = dict(
                            [
                                (av.split("=")[0], av.split("=")[-1])
                                for av in features.split("|")
                            ]
                        )
                        newf = update(mf, newf)
                ...
                # TODO faire ça un jour
                    ...
                    # mate:
                    nr, t, lemma, plemma, _, tag, feat, misc, _, idgov, _, rel, _, _ = (
                        cells
                    )
                    # print(plemma)
                    nr = int(nr)
...
def trees2conllFile(
    trees, outfile, sentencefeatures=True, columns="u"
):  # changed default from 10 to u!
    """
                ...
                if sentencefeatures:
                    for stftkey in sorted(tree.sentencefeatures):
                        if stftkey == "_comments":
                            ...
                            treestring += (
                                stftkey + " = " + tree.sentencefeatures[stftkey]
                            )
