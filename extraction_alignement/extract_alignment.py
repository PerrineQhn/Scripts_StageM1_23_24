            ...
            while tier_index < len(tier_tok):
                ...
                if sent_token.upper() == tier_token.upper():
                    ...
                    if tier_index + 1 < len(tier_tok):
                        if sent_token.upper() == tier_tok[tier_index + 1][0].upper():
                        ...
                        elif sent_token.upper() == "VICE-PRESIDO" and tier_tok[tier_index+1][0].upper() == "VICE" and tier_tok[tier_index + 2][0].upper() == "#":
                            # print(
                            #     "VICE-PRESIDO",
                            #     "sent_token",
                            #     sent_token,
                        ...
                        elif sent_token.upper() != tier_tok[tier_index + 1][0].upper() and tier_tok[tier_index][2] < tier_tok[tier_index + 1][1]:
                            # print(
                            #     "sent_token_3",
                            #     sent_token,
                            #     "tier_token",
                            ...
                            # )
                            xmin_new = f"{int(float(tier_tok[tier_index][2]) * 1000):06d}"
                            xmax_new = f"{int(float(tier_tok[tier_index + 1][1]) * 1000):06d}"
...
def save_alignment_results(sentences, output_file):
    ...
    with open(output_file, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["sent_id", "sent_text", "new_sent_text", "token_info", "syll_info"])
...
def create_textgrid_slam(sentences:dict, output_file:str):
    ...
    for sent_id, sentence_data in sentences.items():
        ...
        for token in token_info:
            ...
            if word_idx < len(words_in_sentence) and words_in_sentence[word_idx].upper() == token[0].upper():
            ...
            if alignbegin is not None and alignend is not None and alignbegin < alignend:
                if word != "#":
...
conllu_path = "/Users/perrine/Desktop/Stage_2023-2024/SUD_Naija-NSC-master"
    ...
    for file in files:
        if file.endswith("_MG.conllu") or file.endswith("_M.conllu"):
            ...
            # print(sentences)
                ...
                output_file = non_gold_textgrid_path + textgrid_name + ".TextGrid"
